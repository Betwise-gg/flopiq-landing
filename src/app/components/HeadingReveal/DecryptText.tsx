...
export default function DecryptedText({
  ...
  // Calculate adaptive speed - ensure minimum 300ms total animation
  // For short words, slow down the per-character speed
  const minTotalDuration = 300; // ms
  const charCount = text.replace(/\s/g, '').length;
  const adaptiveSpeed = Math.max(speed, Math.ceil(minTotalDuration / Math.max(charCount, 1)));
